...
/** Dashed custom-attributes reorder region; highlights when a dragged row hovers another section. */
    ...
    <Box
      sx={({ tokens, palette }) => {
        ...
        return {
          ...
          p: emptyList ? 3 : 2,
          border: "1px dashed",
          borderColor: crossSectionHighlight ? accentBorder : uiDividerDefaultBorderColor(tokens),
          borderRadius: 1,
          overflow: "visible",
          // Avoid MUI `alpha(palette.primary.*)` here: Atlas themes often use CSS variables, which `alpha` cannot parse.
          bgcolor: crossSectionHighlight
            ? tokens.semantic.color.surface?.subtle?.value ?? "action.selected"
            : "transparent",
          boxShadow: crossSectionHighlight ? `0 0 0 2px ${focusRing}` : "none",
        ...
        <Stack direction="row" gap={1} flexShrink={0} flexWrap="wrap" alignItems="center" justifyContent={{ xs: "flex-start", md: "flex-end" }}>
          {showAttributeOrderTools && (
            <Button
              ...
              aria-disabled={inlineReorderActive}
              aria-label={STR.schemaManagement.previewInHost}
            >
              {STR.schemaManagement.previewInHost}
            </Button>
          )}
          {showAttributeOrderTools && (
            <Button
                    ...
                    {reorderActive ? (
                      ...
                      >
                        ...
                        {customRows.length > 0 ? (
                          ...
                          <SectionEmptyDropZone
                            sectionId={section.id}
                            label={STR.schemaManagement.reorderEmptyCustomSection}
                          />
                        )}
                      </ReorderCustomAttributesSurface>
                    ) : (
