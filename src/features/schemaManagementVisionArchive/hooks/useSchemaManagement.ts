...
export function useSchemaManagement(
  ...
  /**
   * Soft-delete for the prototype: marks the attribute removed from active schema use.
   * UI copy treats this as deletion; entries remain visible as "recently deleted" for a period.
   */
  const deleteAttribute = useCallback(
    (id: string, reason?: string, attributeName?: string) => {
      ...
      const changes: ChangeRecord[] = reason
        ...
        : [];
      setAuditLog((prev) => [...prev, createEntry(id, attributeName ?? id, "deleted", changes)]);
