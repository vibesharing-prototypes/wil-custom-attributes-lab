...
/** Accent / ring color used for selection rings and swatch borders. */
...
/**
 * Matches the workflow template preview board: a state is "protected" when any
 * outgoing transition has guards and/or async actions (webhooks, side effects).
 */
export function isProtectedWorkflowState(state: WorkflowStateDraft): boolean {
  return state.transitions.some((t) => t.guards.length > 0 || t.actions.length > 0);
}
